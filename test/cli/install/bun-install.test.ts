...
it("should handle non-standard semver pre-release range, issue#4950", async () => {
  const urls: string[] = [];
  setHandler(dummyRegistry(urls, { "3.0.0": { as: "0.0.3" }, "2.1.0": { as: "0.0.2" } }));
  await writeFile(
    join(package_dir, "package.json"),
    JSON.stringify({
      name: "foo",
      version: "0.0.1",
      dependencies: {
        bar: "1 || 2 || pre-3 || 3", // parses left to right, 2.x.x will match before 3.x.x
      },
    }),
  );
  const { stdout, stderr, exited } = spawn({
